# Code ownership: areas and zones

**"Area" and "zone" are the same thing.** The repo uses both words
interchangeably and defines neither, which is why this page exists.

An **area** (= zone) is one top-level ownership bucket: a directory the repo
treats as a single unit for two purposes — who gets auto-requested for review,
and how wide a single PR is allowed to be. Areas are not a build concept, a
deploy concept, or a runtime concept. They exist only for review scope.
...
| Area | `pr-zone-check` pattern | CODEOWNERS line | Owner |
|---|---|---|---|
...
Every owner is a `@qiuethan` placeholder for now. Per-area assignment happens as
...
|---|---|---|
| [`.github/CODEOWNERS`](../.github/CODEOWNERS) | GitHub auto-requests the area's owner on a matching PR | No — `require_code_owner_reviews` is off, so it requests but does not gate |
| [`pr-zone-check.yml`](../.github/workflows/pr-zone-check.yml) | Warns when one PR touches more than one area; writes the list to the job summary | No — ends in `exit 0` by design |
| [`PULL_REQUEST_TEMPLATE.md`](../.github/PULL_REQUEST_TEMPLATE.md) | Prompts the author to name the area | No |
| [`ISSUE_TEMPLATE/`](../.github/ISSUE_TEMPLATE) | `feature_request` prompts for one area; `epic` prompts for the several it spans | No |
...
**Keep a PR inside one area.** Not because CI stops you, but because a
single-area PR has one obvious reviewer and one obvious blast radius.

Spanning areas is sometimes correct. The sanctioned cases:

- **A `packages/auth` change that ripples into every service's shim.** This is
  expected to span every area — see
...
For epics, list every area in the epic body, then keep each sub-issue and each
...
same area, doing the right thing does not trip the multi-area warning.
...
file matches applies, so an owner added to a specific area line replaces the
`*` fallback rather than adding to it.

**Areas are a review boundary, not an architectural one.** They happen to line
...
August 2026, five areas had no zone entry and fell through to the fallback
...
**A file that matches nothing still has an area.** There is no unowned state.
...
1. [`.github/CODEOWNERS`](../.github/CODEOWNERS) — the area line and its owner.
...
If the new area is a service, its CI job in
[`ci.yml`](../.github/workflows/ci.yml) belongs in that PR too — a service has
shipped to staging with no CI coverage before, and the area list is not what
catches that.
...
**The templates offer 12 of the 14 areas, on purpose.** They list `root` but not
...
right response is to give it a real area
([above](#adding-or-renaming-an-area)), not to pick the catch-all off a menu.
...
**Both names survive, deliberately.** The machinery says "zone" — the workflow
filename, its job name, the PR template heading — so the templates and prose say
"zone" too. `CODEOWNERS` and the README say "area" in their comments. Renaming
either direction would touch a workflow filename for a cosmetic gain, which is a
bad trade the moment that job becomes a required status check. They are the same
thing; saying so once, here, is cheaper than a rename.
...
without changing the odds. A CI step that extracts the areas from `zone_for()`
