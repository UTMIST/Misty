...
The dedicated adapter also checks voice-channel context before changing state. A
start from a different voice channel is refused while Misty is recording, and a
stop from outside the recorded channel is refused with the active channel and
auto-stop guidance. Status always names the recorded channel; when idle it tells
